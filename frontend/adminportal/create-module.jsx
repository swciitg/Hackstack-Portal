...
import { createAdminModule, createAdminQuiz } from "./admin-api";
    ...
    <div className="rounded-xl border border-gray-800 bg-gray-950 p-4">
      <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div>
          ...
          {fileName ? (
            <p className="mt-2 text-xs text-emerald-300">Loaded: {fileName}</p>
          ) : null}
        </div>
...
export default function CreateModule() {
        ...
        chapters: [
          {
            ...
            days: preparedDays.map(({ questions, ...day }) => day),
          },
        ],
      },
      preparedDays,
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const createdModule = await createAdminModule(payload.modulePayload);
      const createdDays = (createdModule.chapters || []).flatMap(
        (chapter) => chapter.days || [],
      );

      if (!createdModule._id || createdDays.length !== payload.preparedDays.length) {
        throw new Error("Module was created, but day IDs were not returned correctly.");
      }
      ...
      await Promise.all(
        payload.preparedDays.map((day, index) =>
          createAdminQuiz({
            ...
            dayId: createdDays[index]._id,
            questions: day.questions,
          }),
        ),
      );

      setSuccess(`Module "${createdModule.title}" was created with ${dayCount} day quizzes.`);
      setModuleNumber(Number(moduleNumber) + 1);
      setModuleName("");
      setLearningPoints("");
      setTempInfo("");
      setFinalAssessment("");
      ...
      <div className="mx-auto max-w-5xl">
        <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <h1 className="mt-1 text-2xl font-bold tracking-tight">Create a Module</h1>
            <p className="mt-1 text-sm text-gray-400">
              Build the user-facing module, daily readings, videos, and quizzes in one pass.
            </p>
          </div>
            ...
            <div className="grid gap-4 md:grid-cols-[160px_1fr_180px]">
              <label className="block">
                ...
                <input
                  ...
                  className="w-full rounded-lg border border-gray-700 bg-gray-950 px-3 py-2.5 text-sm text-white outline-none transition focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/30"
                />
              </label>
                ...
                {day.videoUrls.map((url, videoIndex) => (
                  <div key={videoIndex} className="flex gap-2">
                    <input
                      ...
                      className="min-w-0 flex-1 rounded-lg border border-gray-700 bg-gray-950 px-3 py-2.5 text-sm text-white outline-none transition placeholder:text-gray-600 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/30"
                    />
                    <button
                      type="button"
                  ...
                  >
                    ...
                    <label className="block">
                      ...
                      <input
                        ...
                        className="w-full rounded-lg border border-gray-700 bg-gray-900 px-3 py-2.5 text-sm text-white outline-none transition focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/30"
                      />
                    </label>
          ...
          <div className="sticky bottom-0 border-t border-gray-800 bg-gray-950/95 py-4 backdrop-blur">
            <div className="mx-auto flex max-w-5xl flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              <p className="text-sm text-gray-500">
                This creates one published module and one quiz for each day.
              </p>
              <button
                type="submit"
                disabled={submitting}
                className="rounded-lg bg-indigo-600 px-5 py-2.5 text-sm font-bold text-white shadow-lg shadow-indigo-950/40 transition hover:bg-indigo-500 disabled:cursor-not-allowed disabled:bg-indigo-900 disabled:text-indigo-200"
              >
